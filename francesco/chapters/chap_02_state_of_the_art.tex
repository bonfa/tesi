...
\end{comment}	
  ...
  \section{OpenCV}
	...
	Figure \ref{fig:imgMatrix} shows this concept. The machine does not recognize automatically the side mirror of the driver, it sees just a grid of numbers. A goal of computer vision is to turn on the grid that describes this particular object into the object itself. 
	...
	Some causes of noise are easy to remove. The errors caused by the lens, for example, can generally be mathematically described by polynomial functions, thus it is quite simple to correct them. 
	...
	Differently from the previous example, such variables are not set a priory by the modeler but are learnt from a series of tries. For this reason they can be hidden parameters or may even not represent physical quantities and remain mathematical variables.
